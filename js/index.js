...
//NumPerPage分繰り返して表の中身を追加する
  display.forEach(element => {
    ...
    <tr class="${element.type === '0' ? 'type-word' : 'type-phrase'}">
      <td class="words"><button class="appwords"><span>${element.words}</span></button></td>
      <td class="meanings"><button class="appmeanings"><span>${element.meanings}</span></button></td>
...
function RefreshUI() {
  ...
  total.innerHTML = `全${info.total}件`;
  //現在のページ表示
...
async function ChangeStatus(event) {
  ...
  if (status === 0) {
    ...
    status++;
    change.innerHTML = 'Learned'
  } else if (status === 2) {
    change.classList.replace('learned','learning');
    status = 0;
    change.innerHTML = 'Learning'
  }
  ...
  setTimeout(async () => {
    try {
      ...
      if (data.status === 'error') {
        ...
        statusinform.innerHTML = 'ユーザー名またはパスワードが違います';
      ...
      console.error('データの登録に失敗しました');
      statusinform.innerHTML = "dbに接続できません";
    } finally {
      submit.disabled = false;
    }
  }, 1000);
